  ...
  const menuOptions = [
    {
      id: 1,
      name: "Poll",
      icon: <ChartBarIcon className="h-6 w-6 text-college-dark-white-2" />,
      action: () => {
      ...
      <div className="min-h-screen h-full relative w-full">
        ...
        >
          ...
          <EmojiPicker
            ...
            onEmojiClick={(e) => {
              const curPosition = inputRef.current?.selectionStart;
              if (curPosition === undefined || curPosition == null) return;
              const text = messageText;
              const newText =
                text.slice(0, curPosition) + e.emoji + text.slice(curPosition);
              setMessageText(newText);
              setIsEmojiPicker(false);
            }}
          />
          {isPollMenuOpen && (
            <PollMenu closePollMenu={() => setIsPollMenuOpen(false)} />
          )}
          <form
          ...
          >
            <div className="flex gap-1 items-center">
              <ChartBarIcon
                ...
                onClick={() => setIsPollMenuOpen((prev) => !prev)}
              />
